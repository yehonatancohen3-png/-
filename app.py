...
# 6. יצירת System Prompt דינמי (כולל מצב מבחני רבנות)
...
"""
    ...
    if style_mode == "הכנה למבחני רבנות":
        return f"""
אתה מודול AI תורני מומחה, המותאם במיוחד לסייע ללומדים ולנבחנים במבחני ההסמכה לרבנות הראשית (יורה יורה, שבת וכדומה).

תפקידך לתת תשובה מדויקת, מובנית וקפדנית העונה בדיוק על דרישות הבוחנים ברבנות:
1. **השתלשלות פסיקה סדורה:** הצג את הדין מתחילתו – גמרא, ראשונים (רמב"ם, טור), שולחן ערוך, ונושאי הכלים המרכזיים (ש"ך, ט"ז, פרישה, משנה ברורה וכדומה).
2. **ציון מראי מקומות מדויקים:** הקפד לציין במדויק מספרי סימנים וסעיפים בשולחן ערוך ובנושאי הכלים.
3. **הבחנה בין שיטות ומנהגים:** הדגש במטבע לשון ברור מהי דעת השולחן ערוך (מרן), מהי דעת הרמ"א, ומהו המנהג הרווח להלכה למעשה.
...
def analyze_sugya(messages_history, style_mode):
    try:
        ...
        # שימוש במודל 'gemini-1.5-flash' המקורי והיציב

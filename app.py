...
# פונקציות ליצירה וטעינה של נתוני משתמש
def init_user_data():
...
# פונקציית טעינת מאגר תורני מקומי
@st.cache_data
def load_local_database():
    if os.path.exists(DB_FILE):
        try:
            with open(DB_FILE, 'r', encoding='utf-8') as f:
                # הנחת מבנה: מילון של { "שם_מקור": "טקסט מלא" }
                return json.load(f)
        except Exception as e:
            st.error(f"שגיאה בטעינת המאגר המקומי: {e}")
            return {}
    return {}

local_db = load_local_database()

# אינדקס מילים מהיר (Fast Word Index) לחיפוש RAG מקומי
@st.cache_data
def build_fast_word_index(db):
    index = {}
    for title, text in db.items():
        # פיצול פשוט למילים נקיות
...
def rag_local_search(query, db, index, limit=3):
    """מנוע חיפוש מקומי מבוסס אינדקס מילים (RAG)"""
    if not db:
    ...
    for title, score in top_results:
        snippet = db[title][:500] + "..." # לוקחים תקציר מהמאגר
        results_text += f"\nמקור מקומי: {title}\nתוכן (חלק): {snippet}\n"
...
def clean_html_tags(text):
    """מסיר תגיות HTML מטקסט המגיע מספריא"""
    clean = re.compile('<.*?>')
    return re.sub(clean, '', text)

def search_sefaria(query, limit=3):
    """מבצע חיפוש בספריא ושולף את הטקסט העברי של התוצאות הרלוונטיות."""
